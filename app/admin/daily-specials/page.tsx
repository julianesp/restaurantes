...
const emptyForm = { name: "", description: "", price: "", category: "Almuerzo", image: "" };
...
export default function DailySpecialsPage() {
  ...
  const loadSpecials = async () => {
    try {
      ...
      console.error("Error cargando platos del día:", e);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id: string) => {
    if (!confirm("¿Estás seguro de eliminar este plato del día?")) return;
    await fetch(`/api/daily-specials?id=${id}`, { method: "DELETE" });
    await loadSpecials();
  };

  const toggleAvailability = async (special: DailySpecial) => {
    await fetch("/api/daily-specials", {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ id: special.id, available: !special.available }),
    });
    await loadSpecials();
  };
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <ImageUploader
                  ...
                  onChange={(url) => setFormData({ ...formData, image: url } as any)}
                />
              </div>
          ...
          <div className="grid md:grid-cols-2 gap-6">
            {specials.map((special) => (
              ...
              >
                ...
                {(special as any).image && (
                  <img src={(special as any).image} alt={special.name} className="w-full h-40 object-cover rounded-xl mb-3" />
                )}
                <p className="text-gray-600 dark:text-gray-300 mb-4">{special.description}</p>

                <div className="flex items-center justify-between">
                  <p className="text-2xl font-bold text-primary-500">{special.price}</p>
                  <button
                    onClick={() => toggleAvailability(special)}
                ...
                <p className="text-xs text-gray-500 dark:text-gray-400 mt-4">
                  Creado: {formatDate(special.created_at)}
                  {special.created_by && ` · por ${special.created_by}`}
                </p>
              </div>
            ))}
          </div>
